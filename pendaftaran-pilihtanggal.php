...
if (isset($_GET['id_poli'])) {
    ...
    $sql_nama_poli = "SELECT NAMA_POLI FROM poli WHERE ID_POLI = ?";
    $stmt_nama_poli = $con->prepare($sql_nama_poli);
    $stmt_nama_poli->bind_param("s", $id_poli);
    $stmt_nama_poli->execute();
    $hasil_nama_poli = $stmt_nama_poli->get_result();

    if ($hasil_nama_poli->num_rows > 0) {
        $row_nama_poli = $hasil_nama_poli->fetch_assoc();
        $nama_poli = $row_nama_poli['NAMA_POLI'];
    } else {
        echo "Nama Poli tidak ditemukan.";
        exit;
    }
} else {
                ...
                <div class="container">
                    <div class="title-box text-center">
                        <div class="content-wrapper">
                            <h5 class="title-box-name" style="font-size: 31px;">Pilih Tanggal untuk Poli <?php echo htmlspecialchars($nama_poli); ?></h5>
                    ...
                    <div class="row row-cols-1 row-cols-md-3 g-4 mt-3">
                        ...
                        for ($i = 0; $i < 5; $i++) {
                            ...
                            // Query untuk mendapatkan jumlah antrian pada tanggal tertentu
                            $sql_antrian = "SELECT COUNT(*) as total_antrian FROM antrian WHERE ID_POLI = ? AND HARI_TGL = ?";
                            $stmt_antrian = $con->prepare($sql_antrian);
                            $stmt_antrian->bind_param("ss", $id_poli, $tanggal);
                            $stmt_antrian->execute();
                            $hasil_antrian = $stmt_antrian->get_result();
                            $row_antrian = $hasil_antrian->fetch_assoc();
                            $total_antrian = $row_antrian['total_antrian'];
                        ?>
                            <div class="col">
                                <div class="card" style="border-radius: 11px;">
                                    <div class="card-body">
                                        <h5 class="card-title" style="font-size: 19px;"><?php echo date('l, d F Y', strtotime($tanggal)); ?></h5>
                                        <p class="card-text" style="font-size: 14px;">Total antrian: <?php echo $total_antrian; ?></p>
                                        <a href="pendaftaran-konfirmasi.php?id_poli=<?php echo $id_poli; ?>&tanggal=<?php echo $tanggal; ?>&total_antrian=<?php echo $total_antrian; ?>" class="btn btn-primary">
                                            Pilih Tanggal
                                        </a>
                                    </div>
                                </div>
