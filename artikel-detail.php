...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM artikel_kesehatan WHERE ID_ARTIKEL = '$id'";
    $hasil = mysqli_query($con, $sql);

    if ($hasil && mysqli_num_rows($hasil) > 0) {
        $row = mysqli_fetch_assoc($hasil);
        // Ambil data gambar dari database
        $imageData = $row['FOTO_ARTIKEL']; // Gambar disimpan dalam kolom 'gambar'
        ...
        <main class="container my-5">
            <div class="card mx-auto" style="max-width: 800px;">
                <div class="card-body text-center" style="margin-left: 40px; margin-right: 40px; margin-top: 40px">
                    <h2 class="card-title fw-bold fs-3"><?php echo htmlspecialchars($row['JUDUL_ARTIKEL']); ?></h2>
                    <p class="card-subtitle text-muted mb-3">Ditulis oleh: <?php echo htmlspecialchars($row['PENULIS_ARTIKEL']); ?> | Tanggal: <?php echo date("d M Y", strtotime($row['TGL_ARTIKEL'])); ?></p>
                    <img src="<?php echo $gambarSrc; ?>" class="card-img-top mx-auto" alt="Gambar Artikel" style="width: 500px; height: auto; margin-top: 40px; margin-bottom: 40px">
                    <p class="card-text mt-3" style="text-align: start;"><?php echo nl2br(htmlspecialchars($row['ISI_ARTIKEL'])); ?></p>
                </div>
            </div>
        </main>
    </div>
